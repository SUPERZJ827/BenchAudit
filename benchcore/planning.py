...
CORE_CAPABILITIES: tuple[CheckerCapability, ...] = (
    ...
    CheckerCapability("grounded_rubric_consistency", requires_all=frozenset({ArtifactKind.TASK_SPECIFICATION, ArtifactKind.EVALUATOR}), evidence_level="llm", cost_class="medium", needs_llm=True),
    CheckerCapability("rubric_output_contract_consistency", requires_all=frozenset({ArtifactKind.EVALUATOR, ArtifactKind.OUTPUT_CONTRACT}), evidence_level="llm", cost_class="medium", needs_llm=True),
    CheckerCapability("workspace_artifact_invariants", requires_all=frozenset({ArtifactKind.TASK_SPECIFICATION, ArtifactKind.EVALUATOR}), evidence_level="artifact_replay", cost_class="low"),
    CheckerCapability("workspace_rubric_grounding", requires_all=frozenset({ArtifactKind.TASK_SPECIFICATION, ArtifactKind.EVALUATOR}), evidence_level="llm_verified", cost_class="high", needs_llm=True),
    CheckerCapability("solution_leak", requires_all=frozenset({ArtifactKind.TASK_SPECIFICATION, ArtifactKind.ORACLE}), evidence_level="static"),
...
def build_audit_plan(
    ...
    *,
    ...
    # No classification step: a capability is planned when the package supplies
    # the artifacts it reads, and each checker rules itself out per item against
    # the profile derived from the rows.  Guessing a benchmark's identity from
    # its field names decided both of those from three hard-coded names.
    reasons: list[str] = []
    kinds = package.kinds()
